...
void drawall(gamedata &g){
  ...
  int screenheight = 0;
  if (g.players == 1){
    screenheight = limit(GAME_HEIGHT,GAME_HEIGHT,464);
  }else{
    screenheight = limit(GAME_HEIGHT,GAME_HEIGHT,224);
  }
  // Player 1
  int x1 = limit(int(g.player1->x)-308, 0, GAME_WIDTH-640);
  ...
  display_playinfo(g.virtualscreen, 1, *g.player1, screenheight, g.images, g.mission,
                   g.targetscore, g.whitefont, g.greenfont);


  // Player 2
  if (g.players == 2){
    int x2 = limit(int(g.player2->x)-308, 0, GAME_WIDTH-640);
    int y2 = limit(int(g.player2->y)-54, 0, GAME_HEIGHT-224);
    SDL_Rect srcrect;
      srcrect.x = x2;
      srcrect.y = y2;
      srcrect.w = 640;
  ...
  if (g.planes > g.players){
    ...
    // Display score
    display_score(g.virtualscreen, bestp, 536, screenheight, g.mission, g.targetscore,
